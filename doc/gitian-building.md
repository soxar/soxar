...
*Setup instructions for a Gitian build of Soxar Core using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the Soxar
...
- [Building Soxar Core](#building-soxar-core)
...
Clone the git repositories for Soxar Core and Gitian.
...
in the Soxar Core repository under 'Fetch and create inputs' to install sources which require
...
To build Soxar Core (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the Soxar Core repository.
...
```bash
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/soxar/.git/
    remote: Counting objects: 57959, done.
    remote: Total 57959 (delta 0), reused 0 (delta 0), pack-reused 57958
    Receiving objects: 100% (57959/57959), 53.76 MiB | 484.00 KiB/s, done.
    Resolving deltas: 100% (41590/41590), done.
    From https://github.com/soxarpay/soxar
...
For example:
```bash
URL=https://github.com/crowning-/soxar.git
COMMIT=b616fb8ef0d49a919b72b0388b091aaec5849b96
./bin/gbuild --commit soxar=${COMMIT} --url soxar=${URL} ../soxar/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit soxar=${COMMIT} --url soxar=${URL} ../soxar/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit soxar=${COMMIT} --url soxar=${URL} ../soxar/contrib/gitian-descriptors/gitian-osx.yml
```
...
and the soxar git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=trusty on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../soxar/contrib/gitian-descriptors/*|sort|uniq )
...
BTCPATH=/some/root/path/soxar
SIGPATH=/some/root/path/soxar-detached-sigs

./bin/gbuild --url soxar=${BTCPATH},signature=${SIGPATH} ../soxar/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/soxar-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/soxar-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/soxar-osx-build.assert
```
...
[soxar/gitian.sigs](https://github.com/soxarpay/gitian.sigs/) repository, or if that's not possible to create a pull
request.
