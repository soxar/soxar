...
UniValue gobject(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp  ||
        ...
        throw std::runtime_error(
                ...
                "  vote-conf          - Vote on a governance object by masternode configured in soxar.conf\n"
    ...
    */
    ...
    if(strCommand == "vote-conf")
    {
        ...
        if(!fMnFound) {
            ...
            resultsObj.push_back(Pair("soxar.conf", statusObj));
...
{ //  category              name                      actor (function)         okSafe argNames
  //  --------------------- ------------------------  -----------------------  ------ ----------
    /* Soxar features */
    { "soxar",               "getgovernanceinfo",      &getgovernanceinfo,      true,  {} },
    { "soxar",               "getsuperblockbudget",    &getsuperblockbudget,    true,  {"index"} },
    { "soxar",               "gobject",                &gobject,                true,  {} },
    { "soxar",               "voteraw",                &voteraw,                true,  {} },
