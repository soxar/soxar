...
const char * const BITCOIN_CONF_FILENAME = "soxar.conf";
const char * const BITCOIN_PID_FILENAME = "soxard.pid";
    ...
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            if (mapMultiArgs.count("-debug")) {
                ...
                // "soxar" is a composite category enabling all Soxar-related debug output
                if(ptrCategory->count(std::string("soxar"))) {
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\Soxar
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\Soxar
    // Mac: ~/Library/Application Support/Soxar
    // Unix: ~/.soxar
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Soxar";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/Soxar";
#else
    // Unix
    return pathRet / ".soxar";
#endif
#endif
}
...
void ReadConfigFile(const std::string& confPath)
{
    ...
    if (!streamConfig.good()){
        // Create empty soxar.conf if it does not excist
...

    {
        ...
        {
            // Don't overwrite existing settings so command line settings override soxar.conf
